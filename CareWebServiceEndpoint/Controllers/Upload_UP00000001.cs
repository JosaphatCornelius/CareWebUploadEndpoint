...
namespace CareWebServiceEndpoint.Controllers
{
    ...
    public class Upload_UP00000001 : Controller
    {
        ...
        public async Task<string> Upload_Data()
        {
            ...
            Console.WriteLine(Upload_Check().ToString());

            var request = new HttpRequestMessage(HttpMethod.Post, "https://172.20.12.55/CareWebServiceV5/WSEUploader.asmx?op=Upload_Excel");
            request.Content = new StringContent(Upload_Check().ToString(), System.Text.Encoding.UTF8, "application/soap+xml");
        ...
        protected XElement Upload_Check()
        {
            List<UP00000001Model> tes = new List<UP00000001Model>();

            for (int i = 0; i < 100; i++)
            {
                tes.Add(new UP00000001Model
                {
                    HOLDERID = "D01WI01368",
                                ...
                                new XElement(xs + "complexType",
                                    new XElement(xs + "sequence",
                                        new XElement(xs + "element",
                                            ...
                                            new XElement(xs + "complexType",
                                                new XElement(xs + "sequence",
                                                    tes.SelectMany(UP01Obj => UP01Obj.GetType().GetProperties()
