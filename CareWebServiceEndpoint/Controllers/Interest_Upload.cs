...
namespace CareWebServiceEndpoint.Controllers
{
    ...
    [ApiController]
    public class Interest_Upload : Controller
    {
        private readonly SEAWEBContext _seaWebContext;
        private readonly ARTALEARNContext _artaLearnContext;

        public Interest_Upload(SEAWEBContext seaWebContext, ARTALEARNContext artaLearnContext)
        {
        ...
        public async Task<string> InterestUpload([FromQuery] string ano, [FromBody] List<InterestModel> InterestData)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, "http://172.20.12.55/CareWebServiceV5/WSUnderwriting.asmx?op=SaveListInterest");
                request.Content = new StringContent(ConvertJsonToXML(ano, InterestData).ToString(), System.Text.Encoding.UTF8, "application/soap+xml");
        ...
        protected XElement ConvertJsonToXML<T>(string ano, List<T> UploadedData)
        {
            ...
            XElement body = new XElement(soap + "Body",
                new XElement(tem + "SaveListInterest",
